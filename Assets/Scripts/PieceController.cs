...
public class PieceController : MonoBehaviour {
    ...
    public float setDelay = 0.25f;

    private float GradualTime = 0f;
    ...
    public bool MovePiece(Vector2Int movement)
    {
        for (int i = 0; i < tiles.Length; i++)
        {
            if (!tiles[i].CanTileMove(movement + tiles[i].coordinates))
            {
                ...
                { 
                    Debug.Log(PiecesController.instance.isInput);
                    if(!PiecesController.instance.isInput || isDrop){
                    if(GradualTime > setDelay || isDrop){
                        SetPiece();
                        GradualTime = 0;
                        isCounting = false;
                    }
    ...
    public void Update() {
        if(isCounting){
            GradualTime += Time.deltaTime;
        Debug.Log(GradualTime);
        }
